...
export default function FAQSection() {
    const faqItems: FAQItem[] = [
        ...
        },
        {
            id: 'item-6',
            icon: 'package',
            question: 'How do I track my order sold?',
            answer: 'Once your order ships, you\'ll receive a confirmation email with a tracking number. You can use this number on our website or the carrier\'s website to track your package. You can also view order status and tracking information in your account dashboard under "Order History".',
        },
    ];

    return (
        // bg-blend-darken example: bg-blend-[darken|overlay|lighten|multiply|normal|screen]
        <section className="bg-muted bg-[url('./image/bn2.png')] py-30 bg-blend-screen dark:bg-background">
                ...
                <div className="flex flex-col gap-10 md:flex-row md:gap-16">
                    ...
                    <div className="md:w-2/3">
                        <Accordion type="single" collapsible className="w-full space-y-2">
                            {faqItems.map((item) => (
                                <AccordionItem key={item.id} value={item.id} className="rounded-lg border bg-background px-4 shadow-xs last:border-b">
